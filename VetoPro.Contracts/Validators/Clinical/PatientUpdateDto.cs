    ...
    public PatientUpdateDtoValidator()
    {
        ...
        RuleFor(x => x.ReproductiveStatus)
            ...
            .Must(s => s.Equals("Intact", StringComparison.OrdinalIgnoreCase) ||
                       s.Equals("Neutered", StringComparison.OrdinalIgnoreCase) ||
                       s.Equals("Spayed", StringComparison.OrdinalIgnoreCase))
            .WithMessage("Le statut reproductif doit être 'Intact', 'Neutered', ou 'Spayed'.");

        RuleFor(x => x.DeceasedAt)
            .LessThanOrEqualTo(DateTime.UtcNow).WithMessage("La date du décès ne peut pas être dans le futur.")
            .When(x => x.DeceasedAt.HasValue); // Only validate if provided
